...
**Manual on-device testing (post-CI):** the full unmerged stack was hand-tested on real phones + desktop, end to end — invite QR → redeem → wizard, the no-invite gate, admin-only access, invite revoke, and the entire billing lifecycle (cancel → patron blocked → in-progress game survives → reactivate → admin force-status). This surfaced a handful of real integration/UX bugs that automated tests can't catch — all fixed: a service-worker reload loop, token-refresh wiping page state, the invite being lost through Clerk OAuth, a cancelled-venue tap showing a cryptic error instead of the "venue paused" screen, dead-end wrong-surface routing (now auto-routes between the owner/admin areas), and a CORS gap that broke every `PATCH` from the browser in local dev. Open polish items (settings-page redesign to auto-save, geo-autocomplete region bias, a few cosmetic screens) are catalogued in `MORNING-TEST-REPORT.md §8` — none block launch.
