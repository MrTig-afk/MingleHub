...
1. **Backend** (from repo root) — use the restart script, NOT bare `uvicorn --reload`:
   ```
   ./scripts/dev_backend.ps1            # kills orphan workers, starts fresh (with --reload)
   ./scripts/dev_backend.ps1 -NoReload  # fully deterministic; re-run to pick up edits
   ```
   Why: `uvicorn --reload` on Windows can leave an ORPHAN worker serving STALE code after an
   edit (the reloader says "Reloading..." but the old worker keeps answering). The script does
   a kill-all (uvicorn + spawn_main) → wait-for-port-free → fresh start, which can't serve stale
   code. After any backend edit you're about to test, verify against the LIVE endpoint — don't
   trust `--reload`. (Raw command, if ever needed:)
   ```
...
## 8. Post-testing follow-ups (found during manual testing — NOT yet built)
Small, real issues surfaced while manually testing. Logged here so they aren't lost; none block the current builds.

1. **Dead invite-error message.** `redeem-invite` raises `404 "Invalid or expired invite code"`, but the global 404 handler in `api/index.py` rewrites every 404 body to `"Not found"`, so the friendly message never reaches the user — a legit owner who mistypes a code sees a cryptic "Not found". Fix: return **400** (not 404) for the invite rejection so the handler doesn't clobber it; still leaks nothing (all rejection reasons share one message). Files: `api/routers/dashboard_router.py` (~line 1382), check `api/tests/test_venue_invites.py` expectations.
2. **No way to re-fetch an invite link from the admin list.** The QR/URL only shows on the "Invite Generated" screen; once you leave it, the code is gone from the UI (had to pull it from the DB during testing). Add a **"Copy link"** action on each active row in Outstanding Invites. File: `frontend/src/components/Admin/AdminInvites.jsx`.
4. **`venue_inactive` screen is bare.** The patron "This venue is not currently active. Games are temporarily unavailable" screen (`PatronLanding.jsx` ~line 106) is unstyled plain text — functional but ugly. Beautify in the UI pass (icon, venue name, branding, maybe a "come back soon" tone). Confirmed working during manual testing.

3. **Re-tap during a live game after cancel bounces the player.** `resolve_table_state` (`api/services/lobby_service.py` ~line 183) returns `venue_inactive` *before* the resume-into-your-session check (~line 191), so a mid-game player who re-scans the tag after the venue is cancelled gets kicked to "venue not active" instead of resuming. Polling phones are unaffected; only a re-tap is. **Design call:** consider letting an existing-session re-tap resume even when cancelled (move the resume check above the gate), so a paid-then-cancelled patron isn't kicked mid-round.

5. **Settings dashboard redesign — ✅ DONE (auto-save, venue name admin-managed).** Shipped: the owner Settings page now auto-saves the adult-content toggle on change (optimistic, `Saving… / Saved ✓`), the **Save Settings button is gone**, and **Venue Name is read-only** (shown in the Admin-Managed card). The owner PATCH `/settings` no longer accepts `name` (422 if sent); renames now flow through the **admin venue-config override** (`name` added to the override model + UI, audit-logged). Tests updated (`test_dashboard_settings_billing.py`, `test_admin_ops.py`). *Original note below for history.* — The current page mixes two save models (theme auto-saves via `POST /theme`; name + restrict share one `PATCH /settings` "Save" button glued under the adult-content checkbox, with a dirty-state bug that only lights up for the checkbox). **Owner's decision:** go full **auto-save** — every editable field commits on change with inline `Saving… / Saved ✓` (theme already does this); **remove the "Save Settings" button entirely**; and make **Venue Name read-only** (move it to the admin-managed/read-only card — owners can't rename their venue; only an admin override can). Guard: no empty-name path can exist since it's no longer owner-editable. File: `frontend/src/components/Dashboard/DashboardSettings.jsx` (+ owner settings PATCH no longer needs to accept `name`).
6. **Venue-name autocomplete isn't geo-restricted.** Searching a name returns global results (Germany, US, Poland, etc.) instead of biasing to Melbourne / Australia for the launch market. Add a location/region bias (or country filter) to the geo autocomplete. Files: `api/routers/admin_router.py` geo endpoint + the autocomplete call in `AdminInvites.jsx` / settings.
7. **INVESTIGATE — 2nd phone dropped to "join the next round" during a live cancel/reactivate test.** During #6, a phone in a live game fell into the `phase==='question' && !is_participant` state (`SessionParticipant.jsx` ~line 273) — "A round's in progress — you'll join the next one." The game itself survived the cancel (5 rounds played after); the leave/end-game were button taps. Unclear whether the cancel/reactivate or a transient realtime-channel drop caused the participant-drop. Needs a reproduction to trace (does a round-start during the cancel window exclude a momentarily-disconnected phone?).

   **🔍 Static-analysis findings (2026-06-25 — code-traced, not yet repro'd):**
   - **Trivia participants are enrolled once, at `start_trivia`, from `game_players WHERE left_early = FALSE AND phone_id IS NOT NULL`** (`api/services/trivia_service.py:157`). `is_participant` for the current round is then a membership check against `trivia_participants` (`:748`). So a phone shows "join next round" **iff it was not an active member at the instant that round started.**
   - **Billing cancel/reactivate is exonerated.** `venue_lifecycle_service.py` touches **none** of `game_sessions`, `game_players`, or `trivia_participants` (grep-verified) — it only flips venue status. It is structurally incapable of dropping a participant.
   - **A transient realtime/channel drop is also exonerated as a *persisted* cause.** `left_early` is written in **exactly two** places — `leave_session` (`:826`) and `migrate_host` (`session_service.py:216`) — both reached **only** via an explicit `POST /leave` (`patron_router.py:728`). There is **no idle/disconnect/unmount path** that marks a phone left (the `SessionParticipant` effect cleanup only clears timers — `:78`). A phone that merely loses its websocket stays `left_early=FALSE` and is **auto-enrolled in the very next round** — i.e. the symptom is *self-healing within one round* for a still-active phone.
   - **Therefore the remaining real explanations are billing-independent:** (a) the phone had genuinely tapped **Leave** earlier (or switched tables via `SwitchConfirm`) → "join next round" is then *correct, working-as-designed* behavior; or (b) a **one-round transient** where the phone polled `phase==='question'` for a round it wasn't in and then recovered; or (c) a **phone-identity mismatch** if it re-resolved the tag during the `venue_inactive` window and came back on a different `game_players` row.
   - **Repro plan to disambiguate:** 2 phones in a live Trivia game → cancel the venue mid-round from the dashboard → reactivate → keep playing. Capture, per round-start, the `start_trivia` member SELECT (who was `left_early=FALSE`) and the dropped phone's `phone_id` vs its `game_players` row. If the phone is `left_early=TRUE`, find the `POST /leave` that set it (it WILL be a tap/switch). If it's `left_early=FALSE` yet absent from `trivia_participants`, that's a genuine enrollment race worth a fix. **Recommendation:** add a one-line diagnostic log in `start_trivia` (enrolled phone_ids) + in the state builder (phone_id + is_participant) before chasing a fix — the leading hypothesis (disconnect drops a phone) is already disproven by the code, so a speculative behavior change risks breaking the deliberate "no mid-round join" design.
