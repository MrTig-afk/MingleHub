...
- **Type the URL** into any mobile browser
- **QR code** — encode the same URL with any free QR generator and print it on the table
- **Share the link** — one person opens it and AirDrops / messages it to the table

NFC is the premium physical experience, not a technical requirement.
...
Rounds are drawn by the venue's **theme** (Party Night, Date Night, …), which weights which game types and card packs appear. Host hand-off, players leaving/rejoining, and idle timeout are all handled server-side.

---

## Try it yourself (no NFC hardware needed)
...
3. Enter a name on each phone and start a game from the host phone. All phones at the "table" sync live.
...
(The backend also supports **signed tags** — NTAG 424 DNA with server-side signature + replay-counter verification — for cryptographic proof of physical presence. That's an ops/provisioning upgrade, not required.)
...
2. Create a production Postgres (e.g. Neon) and run `scripts/migrate.py` + the seed scripts against it.
3. Set the environment variables from `api/.env.example` and `frontend/.env.local.example` in Vercel.
4. Point a domain at Vercel (automatic HTTPS) — then your NFC tags/QRs carry `https://your-domain.com/<venue>/<table>`.
...
- **Real-time** — Supabase Realtime broadcast for instant multi-phone sync, reconciled by a 2s poll (eventual consistency with reconciliation).
...
Every change lands through CI (lint + full test suite + production build), with a CI guard that fails the build if dangerous code patterns (`eval`, string-built SQL, …) ever appear.

---

## Status

The platform is feature-complete and CI-green: onboarding, the three games, venue + admin dashboards, invite-controlled rollout, theme engine, and the full billing lifecycle (cancel / suspend / dunning / reactivation) — hand-tested end-to-end on real devices. See `COFOUNDER-REPORT.md` for the detailed build report, edge-case catalogue and roadmap.
